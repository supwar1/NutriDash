...
const NutritionAnalysis: React.FC<NutritionAnalysisProps> = ({
    ...
    const radarData = todayScore
        ? Object.entries(todayScore.categoryScores).map(([key, value]) => {
              // 转换键名为更友好的显示名称
    ...
    // 获取过敏源信息
    const allergenInfo = hasAllergies
        ? userData.allergies.map((allergen: string) => {
              const allergenData = foodRestrictions.find(
                  (fr: any) =>
    ...
    // 获取适用于用户年龄的饮食限制
    const dietaryRestrictions = foodRestrictions
        .filter((fr: any) => fr.type === 'restriction')
        .map((restriction: any) => {
